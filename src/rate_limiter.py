...
State is persisted to disk so quota tracking survives server restarts.
"""
import asyncio
import json
import logging
...
# File to persist daily request timestamps across restarts
_STATE_FILE = os.path.join(os.path.dirname(__file__), ".rate_limiter_state.json")
...
class RateLimiter:
    ...
    Checks limits BEFORE each API call and waits if necessary.
    Persists daily request timestamps to disk so quota survives server restarts.
    Operates at 80% of declared limits to provide a safety buffer.
    """

    def __init__(self, config: RateLimitConfig = None) -> None:
        self.config = config or RateLimitConfig()
        # Sliding window: deque of (timestamp, tokens_used) tuples — in-memory only
        self._minute_window: deque = deque()
        # Daily counter: loaded from disk and persisted on every request
        self._daily_timestamps: list = self._load_daily_timestamps()
        logger.info(
            "RateLimiter initialised. Daily requests used today: %d/%d",
            len(self._daily_timestamps), self.config.requests_per_day
        )

    def _load_daily_timestamps(self) -> list:
        """Load persisted daily timestamps from disk, filtering out old entries."""
        try:
            if os.path.exists(_STATE_FILE):
                with open(_STATE_FILE, "r") as f:
                    data = json.load(f)
                cutoff = time.time() - 86400.0
                timestamps = [ts for ts in data.get("daily_timestamps", []) if ts >= cutoff]
                logger.info("Loaded %d daily timestamps from disk", len(timestamps))
                return timestamps
        except Exception as e:
            logger.warning("Could not load rate limiter state: %s", e)
        return []

    def _save_daily_timestamps(self) -> None:
        """Persist daily timestamps to disk."""
        try:
            with open(_STATE_FILE, "w") as f:
                json.dump({"daily_timestamps": self._daily_timestamps}, f)
        except Exception as e:
            logger.warning("Could not save rate limiter state: %s", e)
    ...
    def _purge_daily_entries(self) -> None:
        ...
        cutoff = time.time() - 86400.0
        before = len(self._daily_timestamps)
        self._daily_timestamps = [ts for ts in self._daily_timestamps if ts >= cutoff]
        if len(self._daily_timestamps) < before:
            self._save_daily_timestamps()
    ...
    async def check_and_wait(self, estimated_tokens: int) -> None:
        ...
        self._purge_daily_entries()
        used_today = len(self._daily_timestamps)
        if used_today >= self.config.requests_per_day:
            raise QuotaExhaustedError(
                f"Daily API quota ({self.config.requests_per_day} requests) has been reached. "
                "The quota resets every 24 hours."
            )

        logger.info("Daily usage: %d/%d requests", used_today, self.config.requests_per_day)
        ...
        while iterations < max_wait_iterations:
            ...
            if self._minute_window:
                ...
                wait_time = max(0.5, (oldest_ts + 61.0) - time.time())
            else:
                wait_time = 1.0
    ...
    def record_request(self, tokens_used: int) -> None:
        """
        Record a completed API request. Persists daily count to disk.
        ...
        self._save_daily_timestamps()  # persist immediately
        logger.debug(
